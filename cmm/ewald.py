...
def long_range_potential(coords: torch.Tensor, q: torch.Tensor, p: torch.Tensor, 
    ...
    k_range = torch.arange(-max_hkl, max_hkl + 1, device=coords.device)
    l_range = torch.arange(-max_hkl, max_hkl + 1, device=coords.device)
    
    # Create all combinations and convert to float64
    all_hkl = torch.cartesian_prod(h_range, k_range, l_range).to(torch.float64)
