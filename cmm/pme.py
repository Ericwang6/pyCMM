...
def spread_Q(N, positions, box, Q, n_mesh, rank):
   # print("[HOST] Grid Zeroed. Starting Spread Kernel...")

    # --- CUDA DEBUG: DEEP INSPECTION REPLICATION ---
    #print("\n=== PYTHON DEBUG: DEEP INSPECTION ===")
    #print(f"Kernel Rank received: {rank}")

    raw_q = Q[0].detach() # Atom 0
    q_val = raw_q[0].item()
    px = raw_q[1].item() if rank >= 1 else 0.0
    # Replicating the logic: Q values 4-8 are stored unscaled in Q tensor?
    q20 = (raw_q[4].item() / 3.0) if rank >= 2 else 0.0

    #print("\n--- Computed Values ---")
    #print(f"q_val: {q_val:.6e}")
    #if rank >= 1:
    #    print(f"p_vec: {raw_q[1:4].tolist()}")
    #print("=======================================")
    ...
    # --- CUDA SYSTEMATIC DEBUG REPLICATION (Atom 0, Stencil 0,0,0) ---
    #print("\n=== CUDA SYSTEMATIC DEBUG (Atom 0, Stencil 0,0,0) ===")
    
    # B-Splines for Stencil 0
    stencil_idx = 0
    shift_val = shifts[0, stencil_idx] 
    u_eval = u0[0] + shift_val

    M = bspline(u_eval)
    dM = bspline_prime(u_eval)
    
    # 1. Charge Term = q * Theta
    theta_val = torch.prod(M).item()
    term_charge = q_val * theta_val

    # 2. Dipole Term = P . Grad(Theta)
    div = torch.tensor([
        dM[0] * M[1] * M[2],
        dM[1] * M[2] * M[0],
        dM[2] * M[0] * M[1]
    ], device=M.device)
    grad_theta = torch.mv(-Nj_Aji_star, div)

    term_dipole = 0.0
    if rank >= 1:
        term_dipole = torch.dot(raw_q[1:4], grad_theta).item()

    # 3. Quad Term
    term_quad = 0.0
    # (Simplified debug for quad omitted to focus on dipole as requested, 
    # but the variable exists in flow)

    total_term = term_charge + term_dipole + term_quad

    #print("\n--- Component Breakdown ---")
    #print(f"Term CHARGE (q*theta): {term_charge:.6e}")
    #print(f"Term DIPOLE (p*grad):  {term_dipole:.6e}")
    #print(f"   > P vector: {raw_q[1:4].tolist() if rank>=1 else '0'}")
    #print(f"   > GradTheta: {grad_theta.tolist()}")
    #print(f"Term QUAD   (Q*hess):  {term_quad:.6e}")
    #print(f"Total Term:            {total_term:.6e}")
    #print("=====================================================")

    # 2. Calculate Theta and Terms (Standard Vectorized Code)
    ...
    # --- DEBUG: Grid Summary ---
    flat_grid = Q_mesh.flatten()
    sum_1000 = flat_grid[:1000].sum().item()
    #print(f"[HOST] DEBUG CHECK: First 1000 grid points sum: {sum_1000:.6e}")
...
def interpolate_to_atoms(phi_grid: torch.Tensor,
    ...
    # --- DEBUG: Match CUDA [GPU] Interpolate Check ---
    #print(f"[PY] Interpolate found Non-Zero grid value! {phi_loc[0, 0].item():.6e} at {grid_i[0, 0].item()}")
    # -------------------------------------------------

    phi_atoms = (theta * phi_loc).sum(dim=1)                
    
    print(f"[PY] Atom 0 Final Phi (No self correction): {phi_atoms[0].item():.6e}")
    ...
    if diff_flag == 2:
        ...
        if rank>=1:
            ...
            # --- DEBUG: Field Interpolation Atom 0 Stencil 0 ---
            # Manually calculate the contribution of the first stencil point to the field
            dval_z = thetaprime_z[0,0].item()
            dval_x = thetaprime_x[0,0].item()
            dval_y = thetaprime_y[0,0].item()
            grid_val = phi_loc[0,0].item()
            
            #print(f"\n[PY] Field Interpolation Atom 0 Stencil 0:")
            #print(f"     Grid Phi: {grid_val:.6e}")
            #print(f"     dTheta (x,y,z): {dval_x:.6e}, {dval_y:.6e}, {dval_z:.6e}")
            #print(f"     Contribution (E = dTheta * Phi):")
            #print(f"       Ex: {dval_x * grid_val:.6e}")
            #print(f"       Ey: {dval_y * grid_val:.6e}")
            #print(f"       Ez: {dval_z * grid_val:.6e}")
            # ---------------------------------------------------
            ...
            print(f"[PY] Atom 0 Final Field (Uncorrected): {E_atoms[0].tolist()}")

        if rank>=2:
            thetaprime2 = stgo[...,4:9] / 3 
...
def compute_pme(coords, box, q ,p, t, kappa, k_max ,rank):
  ...
  long_range_potential, long_range_field, long_range_field_gradient = get_pme_recip(Ck_1, kappa,coords,box,Q,K1,K2,K3, rank, bspline_order=6)
  
  # Self Corrections
  kappa_over_root_pi = kappa / torch.sqrt(torch.tensor(torch.pi))
  
  # 1. Potential Self Correction
  long_range_potential = long_range_potential - 2 * kappa_over_root_pi * q
  ...
  print(f"[PY] Atom 0 Field Self Correction: {field_correction[0].tolist()}")
  
  long_range_field = long_range_field + field_correction

  if rank == 1:
      return long_range_potential, long_range_field
      
  # 3. Gradient Self Correction
  grad_correction = kappa_over_root_pi * (16 * kappa * kappa * kappa * kappa / 5) * t / 3
  print(f"[PY] Atom 0 Field Grad Self Correction: {grad_correction[0].tolist()}")
  long_range_field_gradient = long_range_field_gradient + grad_correction
