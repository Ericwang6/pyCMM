...
def plot_eda_scan(xdata, ene_ref, ene_cmm=None, ax=None, keys=list(), xlabel='k_index', ylabel='Energy (kcal/mol)', xmin=-np.inf, xmax=np.inf):
    if ax is None:
    ...
    if ene_cmm:
        ene_cmm = {key: as_numpy(ene_cmm[key])[mask] for key in ene_cmm}
