...
from .polarization import direct_field_induced_dipole_guess, get_field_dependent_polarizabilities, direct_polarization_guess, compute_product_with_polarization_matrix, compute_product_with_polarization_matrix_2, direct_polarization_guess_dipoles_only
...
class CMM_Pol_Test(ForceField):
    def __init__(self,
                 cutoff_short_range: torch.Tensor=torch.tensor(5.0 / BOHR2ANG, dtype=torch.float64),
                 cutoff_ewald: torch.Tensor=torch.tensor(9.0 / BOHR2ANG, dtype=torch.float64),
                 ewald_tolerance: torch.Tensor=torch.tensor(1e-6, dtype=torch.float64),
                 use_ewald: bool=True, use_lr_dispersion: bool=True, use_polarization: bool=True,
                 max_iterations=400, solve_tolerance: torch.Tensor=torch.tensor(1e-7, dtype=torch.float64)) -> None:
        super().__init__()
        # These are local indices for the atom types, not the actual
        # atom type indices which are decided by the Parameterizer.
        self._types_to_index = {
    ...
    def evaluate(self, cm: CoordinateManager, topology: Topology, params: Parameterizer, reset_grads: bool=False):
        ...
        def M_mm_direct(x: torch.Tensor):
            return direct_polarization_guess_dipoles_only(
                x, polarizabilities
            )
