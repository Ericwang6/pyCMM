...
class SPCfw_ASE(Calculator):
    implemented_properties = ['energy', 'forces', 'stress']
    calculate_numerical_stress = False
    calculate_numerical_forces = False

    def __init__(self, ff: SPCfw, cm: CoordinateManager, topology: Topology, params: Parameterizer,
        ...
        self.atoms = Atoms(
            positions=cm.coords.cpu().detach().numpy() * Bohr,
            cell=cm.box.cpu().detach().numpy() * Bohr,
            pbc=[True, True, True],
            symbols=cm.labels
        )
    ...
    def save_state(self, filename: str):
        ...
        state = {
            'positions': self._cm.coords.cpu().detach().numpy().tolist(),
            'velocities': self.atoms.get_velocities().tolist(),  # Save velocities
            'cell': self._cm.box.cpu().detach().numpy().tolist(),
    ...
    def load_state(cls, filename, ff=None):
        ...
        calculator.atoms.set_pbc([True, True, True])
    ...
    def load_last_state(cls, directory: str, ff=None):
        """
        Load a SPCfw calculator from the most recent saved state file in a directory.
