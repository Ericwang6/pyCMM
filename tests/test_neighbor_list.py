...
def test_nsquared_neighbor_list():
    ...
    pairs_nl_1 = nl_1.get_all_pairs()
    pairs_nl_2 = nl_2.get_all_pairs()
    ...
    assert (nl_1.get_all_pairs().size() != nl_2.get_all_pairs().size())
    nl_1.update(coords_2, cell_2)
    assert torch.all(nl_1.get_all_pairs() == nl_2.get_all_pairs())

    # Now check that we can successfully rebuild when the cell changes size
    nl_1 = NSquaredList(coords_1, cell_1, cutoff)
    new_cell = cell_1 + torch.eye(3, device=device)
...
def test_verlet_neighbor_list():
    ...
    nl_1_verlet = VerletList(coords_1, cell_1, cutoff, padding=torch.tensor(1.5))
    assert torch.equal(nl_1_ref.get_all_pairs(), nl_1_verlet.get_all_pairs())

    # Test full update
    nl_1_verlet.update(coords_2, cell_2)
    assert torch.equal(nl_2_ref.get_all_pairs(), nl_1_verlet.get_all_pairs())
    ...
    nl_verlet = VerletList(coords_2, cell_2, cutoff, padding=torch.tensor(1.0))
    modified_cell = cell_2 - torch.eye(3, device=device) * 0.49
    nl_ref = NSquaredList(coords_2, modified_cell, cutoff)
    nl_verlet.update(coords_2, modified_cell)
    assert torch.equal(nl_ref.get_all_pairs(), nl_verlet.get_all_pairs())
    assert torch.equal(nl_verlet._reference_box, cell_2)

    # Check we properly update the reference box on rebuild
    modified_cell = cell_2 - torch.eye(3, device=device) * 0.51
    nl_ref = NSquaredList(coords_2, modified_cell, cutoff)
    nl_verlet.update(coords_2, modified_cell)
    assert torch.equal(nl_ref.get_all_pairs(), nl_verlet.get_all_pairs())
    ...
    nl_verlet = VerletList(coords_1, cell_1, cutoff, padding=torch.tensor(1.0))
    coords_inside_verlet_cutoff = torch.zeros_like(coords_1)
    factor = 1.0
    while True:
        coords_inside_verlet_cutoff = coords_1 + torch.rand_like(coords_1) * factor
    ...
    modified_cell = cell_1 - torch.eye(3, device=device) * 0.24
    ...
    modified_cell = cell_1 - torch.eye(3, device=device) * 0.26
    nl_ref = NSquaredList(coords_inside_verlet_cutoff, modified_cell, cutoff)
    nl_verlet.update(coords_inside_verlet_cutoff, modified_cell)
    assert torch.equal(nl_ref.get_all_pairs(), nl_verlet.get_all_pairs())
